...
const koPath = 'c:/Share/DASAN/src/app/about/[[...slug]]/page.tsx';
const enPath = 'c:/Share/DASAN/src/app/en/about/[[...slug]]/page.tsx';

function updateFile(path, isEn) {
  let code = fs.readFileSync(path, 'utf8');
  
  const salesRow = isEn ? "['Sales', '79,275', '92,734', '110,191']" : "['매출액', '79,275', '92,734', '110,191']";
  const profitRow = isEn ? "['Operating Profit', '2,389', '6,068', '338']" : "['영업이익', '2,389', '6,068', '338']";
  ...
  code = code.replace(/let salesRow = \[[^\]]+\];/, 'let salesRow = ' + salesRow + ';');
  code = code.replace(/let profitRow = \[[^\]]+\];/, 'let profitRow = ' + profitRow + ';');

  code = code.replace(/let consolidatedBS = \[\s*(?:\[.*?\],\s*)*\[.*?\]\s*\];/, 'let consolidatedBS = ' + conBS + ';');
  code = code.replace(/let separateBS = \[\s*(?:\[.*?\],\s*)*\[.*?\]\s*\];/, 'let separateBS = ' + sepBS + ';');
  code = code.replace(/let consolidatedIS = \[\s*(?:\[.*?\],\s*)*\[.*?\]\s*\];/, 'let consolidatedIS = ' + conIS + ';');
  code = code.replace(/let separateIS = \[\s*(?:\[.*?\],\s*)*\[.*?\]\s*\];/, 'let separateIS = ' + sepIS + ';');

  fs.writeFileSync(path, code);
}

updateFile(koPath, false);
updateFile(enPath, true);
console.log('Success');
